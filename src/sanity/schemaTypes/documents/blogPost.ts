...
export const blogPost = defineType({
  ...
  fields: [
    ...
    defineField({ name: 'author', title: 'Autor', type: 'reference', to: [{ type: 'teamMember' }], group: 'meta' }),
    defineField({ name: 'categories', title: 'Categorías', type: 'array', of: [{ type: 'reference', to: [{ type: 'category' }] }], group: 'meta' }),
    defineField({ name: 'publishedAt', title: 'Fecha de publicación', type: 'datetime', group: 'meta' }),
    ...
    defineField({ name: 'featuredImage', title: 'Imagen destacada', type: 'image', options: { hotspot: true }, group: 'content', fields: [{ name: 'alt', title: 'Alt text', type: 'object', fields: [{ name: 'es', type: 'string', title: 'Español' }, { name: 'en', type: 'string', title: 'English' }] }] }),
    defineField({ name: 'excerpt', title: 'Extracto', type: 'object', group: 'content', fields: [{ name: 'es', title: 'Español', type: 'text', rows: 3, validation: (Rule) => Rule.max(200) }, { name: 'en', title: 'English', type: 'text', rows: 3, validation: (Rule) => Rule.max(200) }] }),
    defineField({ name: 'body', title: 'Contenido', type: 'localePortableText', group: 'content' }),
