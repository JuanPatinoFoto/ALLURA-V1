...
export const caseStudy = defineType({
  ...
  fields: [
    ...
    defineField({ name: 'service', title: 'Servicio', type: 'reference', to: [{ type: 'service' }], group: 'content' }),
    defineField({ name: 'summary', title: 'Resumen', type: 'object', group: 'content', fields: [{ name: 'es', title: 'Español', type: 'text', rows: 3, validation: (Rule) => Rule.max(200) }, { name: 'en', title: 'English', type: 'text', rows: 3, validation: (Rule) => Rule.max(200) }] }),
    ...
    defineField({ name: 'beforeImages', title: 'Imágenes ANTES', type: 'array', group: 'media', of: [{ type: 'image', options: { hotspot: true }, fields: [{ name: 'alt', title: 'Alt text', type: 'object', fields: [{ name: 'es', type: 'string', title: 'Español' }, { name: 'en', type: 'string', title: 'English' }] }] }], validation: (Rule) => Rule.max(4) }),
    defineField({ name: 'afterImages', title: 'Imágenes DESPUÉS', type: 'array', group: 'media', of: [{ type: 'image', options: { hotspot: true }, fields: [{ name: 'alt', title: 'Alt text', type: 'object', fields: [{ name: 'es', type: 'string', title: 'Español' }, { name: 'en', type: 'string', title: 'English' }] }] }], validation: (Rule) => Rule.max(4) }),
