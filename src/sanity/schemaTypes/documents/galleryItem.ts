...
export const galleryItem = defineType({
  ...
  fields: [
    ...
    defineField({ name: 'image', title: 'Imagen', type: 'image', description: "Imagen principal de la galería. Mínimo 800×600px. Formato JPG o WebP recomendado.", options: { hotspot: true }, validation: (Rule) => Rule.required(), fields: [{ name: 'alt', title: 'Alt text', type: 'object', fields: [{ name: 'es', type: 'string', title: 'Español', validation: (Rule) => Rule.required() }, { name: 'en', type: 'string', title: 'English', validation: (Rule) => Rule.required() }] }] }),
