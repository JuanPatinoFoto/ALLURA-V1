...
export function BenefitsForm({ settings, onChange }: { settings: Record<string, unknown>; onChange: (s: Record<string, unknown>) => void }) {
    ...
    <div className="space-y-3">
      ...
      {(s.items ?? []).map((item, i) => (
        <div key={i} className="border border-gray-100 rounded-lg p-3 space-y-2 bg-gray-50">
          <p className="text-xs text-gray-400 font-medium">Tarjeta {i + 1}</p>
          <div>
