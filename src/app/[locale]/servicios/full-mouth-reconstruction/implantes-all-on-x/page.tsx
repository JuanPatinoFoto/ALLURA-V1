...
const SERVICE_SLUG = "implantes-all-on-x";
...
export default async function ImplantesAllOnXPage({ params: { locale } }: { params: { locale: string } }) {
  const content = locale === "en" ? contentEn : contentEs;

  // Try to render from Supabase CMS sections
  const page = await getPageBySlug('/servicios/full-mouth-reconstruction/implantes-all-on-x')
  if (page) {
